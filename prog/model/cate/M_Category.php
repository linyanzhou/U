<?php
    ...
    class M_Category extends Model
    {
        public function __construct($setup)
        {
            ...
            $this->_table  = isset($config['category']['table']) ? $config['category']['table'] : null;
            $this->_fields = isset($config['category']['fields']) ? $config['category']['fields'] : null;
        }
        ...
        public function get_page_list($opts, $cache = 0)
        {
            ...
            if ( is_numeric($len) ) {
                $cond .= " and length(cate_id) = $len ";
            }

            if ($terms) {
                $terms = self::$DBM->add_sql_slashes($terms);
                $cond .= " and ( cate_name like '%$terms%' or cate_id like '$terms%') ";
            }
        ...
        public function get_profile($opts, $cache = 0)
        {
            ...
            $cate_id = isset($opts['cate_id']) ? $opts['cate_id'] : null;
            if ($id <= 0 && ! $cate_id && ! $dir)
            return array();
            ...
            if ($cate_id) {
                $cat_id = self::$DBM->add_sql_slashes($cate_id);
