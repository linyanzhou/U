...
class L_Watermark extends Library {

	public $markPosType = 9;          //水印位置，缺省为右下角位置输出水印

	public $fontFile = '/usr/share/fonts/chinese/TrueType/ukai.ttf';   //字体文件名
	...
	public function __construct($setup, $opts = null){
		...
		$this->_opts = $this->extend($opts, 'watermark');
	...
	public function appendTextMark($markText){
		...
		$text_padding = 1;
		$box = $this->calculateTextBox($this->fontSize,$this->angle,$this->fontFile,$markText);
		$this->mark_width = $box["width"] + $text_padding; 
		$this->mark_height = $box["height"] + $text_padding; 
		...
		//if($this->angle >= 90){
		//	$pos[0] += abs(sin($this->angle))*$this->mark_width;
		//}
		//$pos[1] += abs(cos($this->angle))*$this->mark_height;

		switch ($this->angle){
			case 90 :
				$pos[0]+= $this->mark_width;
				$pos[1]+=$this->mark_height;
				break;
			case 180 :
				$pos[0]+=$this->mark_width;
				//$pos[1]+=$this->mark_height;
				break;
			default:$pos[1]+=$this->mark_height;
		}
		//将文字打印到图片上
